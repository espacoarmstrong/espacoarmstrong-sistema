    ...
    <div v-if="abaAtual === 'horarios'" class="card conteudo">
      <div v-for="dia in horarios" :key="dia.dia_semana" class="linha-horario">
        ...
        <template v-if="!dia.folga">
          <input v-model="dia.horario_inicio" type="time" class="input input-hora" />
          <span>às</span>
          <input v-model="dia.horario_fim" type="time" class="input input-hora" />
        </template>
      </div>
...
.linha-horario { display: flex; align-items: center; gap: 10px; padding: 8px 0; border-bottom: 1px solid var(--border); flex-wrap: wrap; }
