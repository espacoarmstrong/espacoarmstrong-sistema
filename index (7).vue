<template>
  <div>
    ...
    <div class="grade">
      ...
      <NuxtLink v-if="podeAcao('comanda_visualizar')" to="/comandas" class="atalho card">
        <h3>Comandas</h3>
        <p>Pagamentos e status das comandas</p>
      </NuxtLink>
      ...
      <NuxtLink v-if="podeAcao('procedimentos_visualizar')" to="/procedimentos" class="atalho card">
        <h3>Procedimentos</h3>
        <p>Serviços e categorias do salão</p>
      </NuxtLink>
      <NuxtLink v-if="podeAcao('colaboradores_visualizar')" to="/colaboradores" class="atalho card">
        <h3>Colaboradores</h3>
        <p>Equipe, permissões e comissões</p>
      </NuxtLink>
      <NuxtLink v-if="podeAcao('remuneracao_visualizar')" to="/remuneracao" class="atalho card">
        <h3>Remuneração</h3>
        <p>Comissões pendentes e pagas</p>
      </NuxtLink>
    </div>
  </div>
</template>
